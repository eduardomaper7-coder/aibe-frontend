...
function PagoInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!mounted) return;
        setErrorMsg(err instanceof Error ? err.message : 'Error creando la sesión de pago');
        setLoading(false);
      }
    })();
  ...
  const PRICE_MAP: Record<string, number> = { basic: 1.49, business: 2.29, plus: 6.49 };
  const NAME_MAP: Record<string, string> = { basic: 'AIBE Básico', business: 'AIBE Business', plus: 'AIBE Plus' };
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mt-4 flex gap-3">
            <a className="rounded-xl px-4 py-2 bg-slate-900 text-white" href={`/pago?plan=${encodeURIComponent(plan)}`}>Reintentar</a>
            <a className="rounded-xl px-4 py-2 border border-slate-300" href={`/registro?plan=${encodeURIComponent(plan)}`}>Cambiar de cuenta</a>
          </div>
        </>
      ) : (
  ...
  return (
    <aside className="mt-6 rounded-2xl border border-slate-200 p-5 bg-white">
      ...
      <dl className="mt-4 space-y-2 text-sm text-slate-700">
        <div className="flex items-center justify-between"><dt>Plan</dt><dd className="font-medium">{planName}</dd></div>
        <div className="flex items-center justify-between"><dt>Primeros 3 días</dt><dd className="font-medium">{fmt.format(0)}</dd></div>
        <div className="flex items-center justify-between"><dt>Después</dt><dd className="font-medium">{fmt.format(priceMonthly)}/mes</dd></div>
        <div className="mt-3 h-px bg-slate-200" />
        <div className="flex items-center justify-between text-base"><dt className="font-semibold">A pagar ahora</dt><dd className="font-extrabold">{fmt.format(0)}</dd></div>
      </dl>
    </aside>
  );
}
