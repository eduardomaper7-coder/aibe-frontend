    ...
    <div className="min-h-screen bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
      <div className="px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-gray-900">Análisis de sentimientos</h1>
          </div>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-5">
              ...
              <div className="md:col-span-2 flex flex-col gap-4">
                <div className="rounded-2xl border bg-white p-4 shadow-sm">
                  <p className="text-sm text-gray-500">Número de reseñas analizadas</p>
                  <p className="mt-1 text-2xl font-semibold text-gray-900">{totalResenas.toLocaleString()}</p>
                </div>
                ...
                <div className="rounded-2xl border bg-white p-4 shadow-sm">
                  ...
                  <div className="mt-1 flex items-end gap-2">
                    <p className="text-2xl font-semibold text-gray-900">{puntuacionMedia.toFixed(1)}</p>
                    <span className="text-sm text-gray-500">/ 5</span>
                  </div>
                  <div className="mt-2 flex items-center gap-2 text-xs">
                    <span className="inline-block h-2 w-2 rounded-full bg-emerald-500" />
                    <span className="text-gray-500">Basado en los últimos 6 meses</span>
                  </div>
                </div>
              ...
              {dataSentimiento.map((item, i) => (
                <div key={item.name} className="flex items-center gap-3 rounded-xl border bg-white p-3 shadow-sm">
                  ...
                  <div className="flex w-full items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-gray-900">{porcentaje(item.value)}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Nota */}
        <p className="mt-4 text-xs text-gray-500">*Este es un ejemplo con datos ficticios. Integra tu clasificador de sentimiento para resultados reales.</p>
      </div>
    </div>
  );
}